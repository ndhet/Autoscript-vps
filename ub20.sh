...
NAMECOM=$(curl -sS https://raw.githubusercontent.com/ndhet/ip/main/ip | grep $MYIP | awk '{print $2}')
...
ftTunneling() {
    ...
    IZIN=$(curl -sS https://raw.githubusercontent.com/ndhet/ip/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
        TIMEDATE
    else
